...
TEST_CASE("CompressedKVStore single-chunk append + flat matches source bytes",
          "[store][flat]") {
...
TEST_CASE("CompressedKVStore multi-chunk flat concatenates per head correctly",
          "[store][flat]") {
    using Store = tq::CompressedKVStore<3, 2>;
    ...
    REQUIRE(Val::dequantize(f.val_data, f.val_scales, f.val_zeros,
                            H * T, D, gs,
                            std::span<float>(v_rt_ht)) == tq::Error::Ok);
    // Find max value scale across the flat.
    float max_sc = 0.0f;
    for (float s : f.val_scales) max_sc = std::max(max_sc, s);
...
TEST_CASE("KVCaptureEngine: decode drains ring on overflow",
          "[engine][decode]") {
    using Eng = tq::KVCaptureEngine<3, 2>;
    ...
    const auto v = make_gaussian(T * rs, 2);
    for (std::size_t t = 0; t < T; ++t) {
        REQUIRE(eng.ingest_decode(
            std::span<const float>(k.data() + t * rs, rs),
            std::span<const float>(v.data() + t * rs, rs), 1) == tq::Error::Ok);
    }
    REQUIRE(eng.total_tokens() == T);
...
TEST_CASE("KVCaptureEngine: flush drains ring into store",
          "[engine][flush]") {
    using Eng = tq::KVCaptureEngine<3, 2>;
...
TEST_CASE("KVCaptureEngine: bulk decode matching interleaved decode",
          "[engine][decode]") {
    using Eng = tq::KVCaptureEngine<3, 2>;
