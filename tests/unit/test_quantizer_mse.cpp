...
TEST_CASE("TurboQuantMSE: quantize is deterministic for fixed seed", "[quantizer][mse]") {
    ...
    std::vector<float> x(batch * d);
    for (std::size_t i = 0; i < x.size(); ++i)
        x[i] = std::cos(0.09f * static_cast<float>(i));
...
TEST_CASE("TurboQuantMSE: reconstruction MSE decreases with bits", "[quantizer][mse]") {
...
TEST_CASE("TurboQuantMSE: shape mismatch surfaces as ShapeMismatch", "[quantizer][mse]") {
