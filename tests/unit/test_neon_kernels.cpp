...
TEST_CASE("neon::scale is bit-exact vs scalar (single multiply per lane)",
          "[neon]") {
...
TEST_CASE("neon::searchsorted_one matches scalar for bits 1..4 codebooks",
          "[neon]") {
...
TEST_CASE("neon::searchsorted_one handles exact-boundary inputs (right=False)",
          "[neon]") {
...
TEMPLATE_TEST_CASE_SIG("neon::searchsorted_and_pack is bit-exact vs scalar",
                      "[neon][pack]",
    ...
    for (std::size_t i = 0; i < n_bounds; ++i) {
        bounds[i] = -0.5f + static_cast<float>(i + 1) /
                    static_cast<float>(n_bounds + 1);
    }
    ...
    tq::neon_scalar::searchsorted_and_pack<Bits>(
        rotated.data(), bounds.data(), n_bounds, ps.data(), d);
    tq::neon::searchsorted_and_pack<Bits>(
        rotated.data(), bounds.data(), n_bounds, pn.data(), d);
    REQUIRE(ps == pn);
}

TEMPLATE_TEST_CASE_SIG("neon::unpack_and_gather is bit-exact vs scalar",
                      "[neon][pack]",
                      ((int Bits), Bits), 1, 2, 3, 4) {
    const std::size_t d = 128;
    ...
    for (std::size_t i = 0; i < n_cl; ++i) {
        centroids[i] = -1.0f + 2.0f * static_cast<float>(i) /
                       static_cast<float>(n_cl - 1);
    }
    ...
    for (auto& r : raw) r = static_cast<std::uint8_t>(dist(rng));
...
TEST_CASE("neon::qjl_unpack_pm1 is bit-exact vs scalar", "[neon][qjl]") {
    for (std::size_t d : {8u, 37u, 128u, 257u}) {
        ...
        for (auto& b : packed) b = static_cast<std::uint8_t>(dist(rng));
