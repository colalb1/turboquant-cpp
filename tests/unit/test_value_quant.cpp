...
TEMPLATE_TEST_CASE_SIG("ValueCodec round-trip is within one level of error",
                       "[value][roundtrip]",
                       ((int Bits), Bits), 2, 4, 8) {
    ...
    REQUIRE(Codec::quantize(v, batch, dim, gs,
                            data, scales, zeros) == tq::Error::Ok);

    std::vector<float> v_rt(batch * dim);
    REQUIRE(Codec::dequantize(data, scales, zeros, batch, dim, gs, v_rt)
            == tq::Error::Ok);
...
TEST_CASE("ValueCodec<2> matches explicit Python packing layout",
          "[value][pack]") {
    ...
    for (std::size_t i = 0; i < dim; ++i) v[i] = static_cast<float>(i % 4);

    std::vector<std::uint8_t> data(tq::ValueCodec<2>::packed_bytes(dim));
    std::vector<float> scales(1), zeros(1);
    REQUIRE(tq::ValueCodec<2>::quantize(v, 1, dim, gs, data, scales, zeros)
            == tq::Error::Ok);
    ...
    for (std::uint8_t byte : data) REQUIRE(byte == 0xE4);
}

TEST_CASE("ValueCodec rejects dim not divisible by group_size",
          "[value][shape]") {
    std::vector<float> v(30);
    std::vector<std::uint8_t> data(30);
    std::vector<float> scales(1), zeros(1);
    REQUIRE(tq::ValueCodec<2>::quantize(v, 1, /*dim=*/30, /*gs=*/32,
                                        data, scales, zeros)
            == tq::Error::InvalidDim);
}

TEST_CASE("neon::group_quant_row matches scalar on random input",
          "[neon][value]") {
    for (int n_levels : {3, 15, 255}) {
        for (std::size_t gs : {8u, 16u, 32u}) {
            ...
            tq::neon_scalar::group_quant_row(v.data(), dim, gs, n_levels,
                                             is.data(), ss.data(), zs.data());
            tq::neon::group_quant_row       (v.data(), dim, gs, n_levels,
                                             in.data(), sn.data(), zn.data());
...
TEST_CASE("neon::group_dequant_row matches scalar on random input",
          "[neon][value]") {
    ...
    for (std::size_t gs : {8u, 16u, 32u}) {
        ...
        for (auto& v : idx)    v = static_cast<std::uint8_t>(di(rng));
        for (auto& v : scales) v = std::fabs(nd(rng)) + 1e-3f;
        for (auto& v : zeros)  v = nd(rng);

        std::vector<float> xs(dim), xn(dim);
        tq::neon_scalar::group_dequant_row(idx.data(), dim, gs,
                                           scales.data(), zeros.data(), xs.data());
        tq::neon::group_dequant_row       (idx.data(), dim, gs,
                                           scales.data(), zeros.data(), xn.data());
        for (std::size_t i = 0; i < dim; ++i) {
            CAPTURE(gs, i);
            REQUIRE(xs[i] == xn[i]);
        }
