...
void reference_attention(const std::vector<float>& query, std::size_t n_q, std::size_t Q,
                         const std::vector<float>& k_all, const std::vector<float>& v_all,
                         std::size_t H, std::size_t N, std::size_t D, float scale,
...
std::vector<float> thd_to_htd(const std::vector<float>& src, std::size_t T, std::size_t H,
                              std::size_t D) {
...
void dequant_flat(tq::CompressedKVStore<KeyBits, ValBits>& store, std::vector<float>& k_out,
    ...
    REQUIRE(store.quantizer().dequantize(
                flat.mse_indices, flat.qjl_signs, flat.residual_norms, flat.norms, H * T,
                std::span<float>(k_out.data(), H * T * D)) == tq::Error::Ok);
    REQUIRE(tq::ValueCodec<ValBits>::dequantize(
                flat.val_data, flat.val_scales, flat.val_zeros, H * T, D, store.value_group_size(),
...
TEST_CASE("compute_hybrid_attention with no history and no recent is zero", "[score]") {
    ...
    REQUIRE(tq::compute_hybrid_attention<4, 4>(query, n_q, Q, store, {}, {}, 0, 0.0f, actual) ==
            tq::Error::Ok);
...
TEST_CASE("compute_hybrid_attention with < MIN_HISTORY and no recent is zero", "[score]") {
    const std::size_t D = 32, H = 2, Q = 4, n_q = 1, gs = 16;
    using Store = tq::CompressedKVStore<4, 4>;
...
TEST_CASE("compute_hybrid_attention compressed-only matches reference", "[score]") {
...
TEST_CASE("compute_hybrid_attention compressed-only across multiple chunks", "[score]") {
    ...
    std::vector<float> actual(n_q * Q * D);
    REQUIRE(tq::compute_hybrid_attention<4, 4>(query, n_q, Q, store, {}, {}, 0, scale, actual) ==
            tq::Error::Ok);
...
TEST_CASE("compute_hybrid_attention recent-only matches fp32 reference", "[score]") {
    const std::size_t D = 32, H = 2, Q = 4, n_q = 1, gs = 16;
    ...
    std::vector<float> actual(n_q * Q * D);
    REQUIRE(tq::compute_hybrid_attention<4, 4>(query, n_q, Q, store, recent_k, recent_v, N_rec,
                                               0.0f, actual) == tq::Error::Ok);
...
TEST_CASE("compute_hybrid_attention uses recent-only when history < MIN_HISTORY", "[score]") {
    const std::size_t D = 16, H = 2, Q = 4, n_q = 1, gs = 8;
...
TEST_CASE("compute_hybrid_attention hybrid concats compressed + recent", "[score]") {
    const std::size_t D = 32, H = 2, Q = 4, n_q = 1, gs = 16;
    ...
    for (std::size_t h = 0; h < H; ++h) {
        std::copy_n(k_dq.data() + h * T_hist * D, T_hist * D, k_ref.data() + h * N_all * D);
        std::copy_n(v_dq.data() + h * T_hist * D, T_hist * D, v_ref.data() + h * N_all * D);
...
TEST_CASE("compute_hybrid_attention rejects mismatched GQA shapes", "[score]") {
    ...
    REQUIRE(tq::compute_hybrid_attention<4, 4>(q, 1, 3, store, {}, {}, 0, 0.0f, o) ==
            tq::Error::InvalidArgument);
}
...
TEST_CASE("compute_hybrid_attention rejects shape mismatches", "[score]") {
    ...
    REQUIRE(tq::compute_hybrid_attention<4, 4>(q_bad, 1, Q, store, {}, {}, 0, 0.0f, o) ==
    ...
    REQUIRE(tq::compute_hybrid_attention<4, 4>(q, 1, Q, store, rec_k_bad, rec_v, 4, 0.0f, o) ==
            tq::Error::ShapeMismatch);
}
