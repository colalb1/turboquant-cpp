...
TEST_CASE("TurboQuantProd: quantize is deterministic for fixed seed", "[quantizer][prod]") {
...
TEST_CASE("TurboQuantProd: attention_score is unbiased when averaged over S seeds",
    ...
    for (std::size_t q = 0; q < n_q; ++q) {
        for (std::size_t k = 0; k < n_k; ++k) {
            ...
            for (std::size_t i = 0; i < d; ++i) {
                acc += static_cast<double>(query[q * d + i]) * static_cast<double>(keys[k * d + i]);
            }
            truth[q * n_k + k] = acc;
        }
    }
    ...
    for (std::uint32_t s = 0; s < n_seeds; ++s) {
        ...
        REQUIRE(prod->attention_score(query, n_q, mi, si, res, norms, n_k, sc) == tq::Error::Ok);
...
TEST_CASE("TurboQuantProd::attention_score matches brute-force on unbiased estimator",
    ...
    for (std::size_t q = 0; q < n_q; ++q) {
        for (std::size_t k = 0; k < n_k; ++k) {
            ...
            for (std::size_t i = 0; i < d; ++i) {
                acc +=
                    static_cast<double>(query[q * d + i]) * static_cast<double>(k_hat[k * d + i]);
            }
    ...
    for (std::size_t q = 0; q < n_q; ++q) {
        for (std::size_t k = 0; k < n_k; ++k) {
            CAPTURE(q, k);
            REQUIRE_THAT(scores[q * n_k + k], WithinAbs(scores_ref[q * n_k + k], 1e-3f));
        }
    }
}

TEST_CASE("TurboQuantProd: shape mismatch surfaces as ShapeMismatch", "[quantizer][prod]") {
...
TEST_CASE("TurboQuantProd::from_matrices accepts external Pi and S", "[quantizer][prod][fixture]") {
    ...
    std::vector<float> s(seed_prod->s_matrix().begin(), seed_prod->s_matrix().end());
