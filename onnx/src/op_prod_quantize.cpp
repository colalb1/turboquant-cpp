...
struct ProdQuantKernel {
    ...
    void Compute(OrtKernelContext* context) {
        ...
        switch (bits) {
        case 2:
            e = s2->quantize({x_data, batch * dim}, batch, {m_p, batch * mb}, {q_p, batch * qb},
                             {r_p, batch}, {n_p, batch});
            break;
        case 3:
            e = s3->quantize({x_data, batch * dim}, batch, {m_p, batch * mb}, {q_p, batch * qb},
                             {r_p, batch}, {n_p, batch});
            break;
        case 4:
            e = s4->quantize({x_data, batch * dim}, batch, {m_p, batch * mb}, {q_p, batch * qb},
                             {r_p, batch}, {n_p, batch});
            break;
        }
        if (e != Error::Ok)
