...
Map&                   global_map()   noexcept { static Map m;                  return m; }
std::shared_mutex&     global_mutex() noexcept { static std::shared_mutex mu;   return mu; }

// Per-thread 2-slot cache — last-used slot rotates.
struct TlsSlot { StateKey key{}; std::shared_ptr<void> ptr; bool valid = false; };
...
template std::shared_ptr<const TurboQuantMSE<1>> get_state<TurboQuantMSE<1>>(const StateKey&) noexcept;
template std::shared_ptr<const TurboQuantMSE<2>> get_state<TurboQuantMSE<2>>(const StateKey&) noexcept;
template std::shared_ptr<const TurboQuantMSE<3>> get_state<TurboQuantMSE<3>>(const StateKey&) noexcept;
template std::shared_ptr<const TurboQuantMSE<4>> get_state<TurboQuantMSE<4>>(const StateKey&) noexcept;
template std::shared_ptr<const TurboQuantProd<2>> get_state<TurboQuantProd<2>>(const StateKey&) noexcept;
template std::shared_ptr<const TurboQuantProd<3>> get_state<TurboQuantProd<3>>(const StateKey&) noexcept;
template std::shared_ptr<const TurboQuantProd<4>> get_state<TurboQuantProd<4>>(const StateKey&) noexcept;
