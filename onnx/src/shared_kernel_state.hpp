...
struct StateKey {
    ...
    bool operator==(const StateKey& o) const noexcept {
        return codec == o.codec && dim == o.dim && bits == o.bits && seed == o.seed;
    }
};
...
struct StateKeyHash {
    std::size_t operator()(const StateKey& k) const noexcept {
        std::uint64_t h =
            static_cast<std::uint64_t>(k.codec) << 56 ^ static_cast<std::uint64_t>(k.dim) << 32 ^
            static_cast<std::uint64_t>(k.bits) << 24 ^ static_cast<std::uint64_t>(k.seed);
...
// header.
extern template std::shared_ptr<const TurboQuantMSE<1>>
get_state<TurboQuantMSE<1>>(const StateKey&) noexcept;
extern template std::shared_ptr<const TurboQuantMSE<2>>
get_state<TurboQuantMSE<2>>(const StateKey&) noexcept;
extern template std::shared_ptr<const TurboQuantMSE<3>>
get_state<TurboQuantMSE<3>>(const StateKey&) noexcept;
extern template std::shared_ptr<const TurboQuantMSE<4>>
get_state<TurboQuantMSE<4>>(const StateKey&) noexcept;
extern template std::shared_ptr<const TurboQuantProd<2>>
get_state<TurboQuantProd<2>>(const StateKey&) noexcept;
extern template std::shared_ptr<const TurboQuantProd<3>>
get_state<TurboQuantProd<3>>(const StateKey&) noexcept;
extern template std::shared_ptr<const TurboQuantProd<4>>
