...
struct MseQuantKernel {
    ...
    MseQuantKernel(const OrtApi& api, const OrtKernelInfo* info) {
        ...
        switch (bits) {
        case 1:
            s1 = get_state<TurboQuantMSE<1>>(k);
            if (!s1) ORT_CXX_API_THROW("MSE_Quantize: state init failed", ORT_RUNTIME_EXCEPTION);
            break;
        case 2:
            s2 = get_state<TurboQuantMSE<2>>(k);
            if (!s2) ORT_CXX_API_THROW("MSE_Quantize: state init failed", ORT_RUNTIME_EXCEPTION);
            break;
        case 3:
            s3 = get_state<TurboQuantMSE<3>>(k);
            if (!s3) ORT_CXX_API_THROW("MSE_Quantize: state init failed", ORT_RUNTIME_EXCEPTION);
            break;
        case 4:
            s4 = get_state<TurboQuantMSE<4>>(k);
            if (!s4) ORT_CXX_API_THROW("MSE_Quantize: state init failed", ORT_RUNTIME_EXCEPTION);
            break;
        }
    }
    ...
    void Compute(OrtKernelContext* context) {
        ...
        auto idx_shape       = shape_with_last(shape_leading(shape), static_cast<std::int64_t>(pb));
        ...
        switch (bits) {
        case 1:
            e = s1->quantize({x_data, batch * dim}, batch, {idx_p, batch * pb}, {nrm_p, batch});
            break;
        case 2:
            e = s2->quantize({x_data, batch * dim}, batch, {idx_p, batch * pb}, {nrm_p, batch});
            break;
        case 3:
            e = s3->quantize({x_data, batch * dim}, batch, {idx_p, batch * pb}, {nrm_p, batch});
            break;
        case 4:
            e = s4->quantize({x_data, batch * dim}, batch, {idx_p, batch * pb}, {nrm_p, batch});
            break;
        }
        if (e != Error::Ok)
