...
struct ValueQuantKernel {
    ...
    void Compute(OrtKernelContext* context) {
        ...
        if (shape.empty() || shape.back() != static_cast<std::int64_t>(dim))
            ORT_CXX_API_THROW("Value_Quantize: last dim must equal head_dim", ORT_INVALID_ARGUMENT);
        ...
        switch (bits) {
        case 2:
            e = ValueCodec<2>::quantize({v_p, batch * dim}, batch, dim, group_size,
                                        {d_p, batch * pb}, {s_p, batch * ng}, {z_p, batch * ng});
            break;
        case 4:
            e = ValueCodec<4>::quantize({v_p, batch * dim}, batch, dim, group_size,
                                        {d_p, batch * pb}, {s_p, batch * ng}, {z_p, batch * ng});
            break;
        case 8:
            e = ValueCodec<8>::quantize({v_p, batch * dim}, batch, dim, group_size,
                                        {d_p, batch * pb}, {s_p, batch * ng}, {z_p, batch * ng});
            break;
        }
        if (e != Error::Ok)
