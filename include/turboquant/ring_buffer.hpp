...
class RingBuffer {
 public:
    static Result<RingBuffer> make(std::size_t capacity,
    ...
    Error write(std::span<const float> keys,
    ...
    void reset() noexcept { pos_ = 0; total_written_ = 0; }
 ...
 private:
    RingBuffer(std::size_t cap, std::size_t nh, std::size_t d,
               AlignedBuffer<float> k, AlignedBuffer<float> v) noexcept
        : capacity_(cap), num_kv_heads_(nh), head_dim_(d),
