...
class CompressedKVStore {
 public:
    static_assert(KeyBits >= 2 && KeyBits <= 4,
                  "CompressedKVStore: KeyBits must be in [2, 4]");
    ...
    static Result<CompressedKVStore>
    make(std::size_t head_dim, std::size_t num_kv_heads,
         std::size_t value_group_size, std::uint32_t seed) noexcept;
    ...
    Error append_chunk(std::span<const float> keys,
 ...
 private:
    ...
    CompressedKVStore(std::size_t head_dim, std::size_t num_kv_heads,
                      std::size_t value_group_size, Prod prod) noexcept
        : head_dim_(head_dim), num_kv_heads_(num_kv_heads),
          value_group_size_(value_group_size), prod_(std::move(prod)) {}
