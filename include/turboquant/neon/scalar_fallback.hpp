...
{
    for (std::size_t i = 0; i < d; ++i) y[i] = x[i] * inv_scale;
}
...
template <int Bits>
inline void searchsorted_and_pack(const float* rotated,
                                  ...
                                  std::uint8_t* packed_out, std::size_t d) noexcept
{
    ...
    for (std::size_t i = 0; i < d; ++i) raw[i] = searchsorted(bounds, n_bounds, rotated[i]);
    Pack::pack(raw, d, packed_out);
}

// Unpack `d` indices and gather centroids.
template <int Bits>
inline void unpack_and_gather(const std::uint8_t* packed,
...
inline void qjl_pack_signs(const float* projected, std::size_t d,
...
inline void qjl_unpack_pm1(const std::uint8_t* packed, std::size_t d,
                           float* out) noexcept
{
...
inline void group_quant_row(const float* x, std::size_t dim, std::size_t gs,
                            int n_levels,
                            std::uint8_t* idx_raw,
...
inline void group_dequant_row(const std::uint8_t* idx_raw,
