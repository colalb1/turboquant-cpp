...
l2norm(const float* x, std::size_t d) noexcept
{
#if defined(__ARM_NEON)
...
scale(const float* x, float inv_scale, float* y, std::size_t d) noexcept
{
#if defined(__ARM_NEON)
...
searchsorted_one(const float* bounds, std::size_t n, float v) noexcept
{
#if defined(__ARM_NEON)
    if (n == 0) return 0;
    ...
    for (; i < n; ++i) count += (bounds[i] < v) ? 1u : 0u;
...
template <int Bits>
[[gnu::always_inline]] inline void
searchsorted_and_pack(const float* rotated,
                      const float* bounds, std::size_t n_bounds,
...
template <int Bits>
[[gnu::always_inline]] inline void
unpack_and_gather(const std::uint8_t* packed,
                  const float* centroids,
                  float* y_out, std::size_t d) noexcept
{
    using Pack = PackPolicy<Bits>;
    std::uint8_t raw[1024];
    Pack::unpack(packed, d, raw);
    for (std::size_t i = 0; i < d; ++i) y_out[i] = centroids[raw[i]];
}
...
// -----------------------------------------------------------------------------
[[gnu::always_inline]] inline void
qjl_pack_signs(const float* projected, std::size_t d,
...
// -----------------------------------------------------------------------------
[[gnu::always_inline]] inline void
qjl_unpack_pm1(const std::uint8_t* packed, std::size_t d,
               float* out) noexcept
{
#if defined(__ARM_NEON)
    const std::size_t nb = QJLPack::packed_bytes(d);
...
// -----------------------------------------------------------------------------
[[gnu::always_inline]] inline void
group_quant_row(const float* x, std::size_t dim, std::size_t gs,
                int n_levels,
                std::uint8_t* idx_raw,
                float* scales, float* zeros) noexcept
{
    ...
    for (std::size_t g = 0; g < ng; ++g) {
        ...
        for (; i + 4 <= gs; i += 4) {
            ...
            vst1_lane_u32(reinterpret_cast<std::uint32_t*>(&packed4),
...
// -----------------------------------------------------------------------------
[[gnu::always_inline]] inline void
group_dequant_row(const std::uint8_t* idx_raw,
                  std::size_t dim, std::size_t gs,
                  const float* scales, const float* zeros,
                  float* x_out) noexcept
{
#if defined(__ARM_NEON)
    const std::size_t ng = dim / gs;
    for (std::size_t g = 0; g < ng; ++g) {
