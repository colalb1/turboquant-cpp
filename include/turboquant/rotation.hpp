...
class Rotation {
 public:
    ...
    [[nodiscard]] TQ_API static Result<Rotation> make(std::size_t dim, std::uint32_t seed) noexcept;
    ...
    [[nodiscard]] TQ_API static Result<Rotation> from_matrix(std::span<const float> pi_row_major,
                                                             std::size_t            dim) noexcept;

    // Forward: y = x @ Pi^T (equivalent to y[i] = Σ_j Pi[i,j] * x[j]).
    // x, y are row-major (batch, dim). Writes exactly batch*dim floats to y.
    [[nodiscard]] TQ_API Error forward(std::span<const float> x, std::span<float> y,
                                       std::size_t batch) const noexcept;

    // Backward: x = y @ Pi (inverse of forward; Pi is orthogonal).
    [[nodiscard]] TQ_API Error backward(std::span<const float> y, std::span<float> x,
                                        std::size_t batch) const noexcept;
