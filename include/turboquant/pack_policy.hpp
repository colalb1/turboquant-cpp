...
struct PackPolicy<1> {
    ...
    static void pack(const std::uint8_t* indices, std::size_t d, std::uint8_t* out) noexcept {
    ...
    static void unpack(const std::uint8_t* packed, std::size_t d, std::uint8_t* out) noexcept {
        const std::size_t nb = packed_bytes(d);
        for (std::size_t b = 0; b < nb; ++b) {
