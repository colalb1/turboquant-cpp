...
Result<Rotation> Rotation::make(std::size_t dim, std::uint32_t seed) noexcept {
    ...
    if (!work.resize(static_cast<std::size_t>(lwork)))
    ...
    vDSP_mtrans(pi.data(), 1, rm.data(), 1, static_cast<vDSP_Length>(dim),
...
Error Rotation::forward(std::span<const float> x, std::span<float> y,
