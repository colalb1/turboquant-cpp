...
inline void transpose_thd_to_htd(const float* src, float* dst,
                                 std::size_t T, std::size_t H, std::size_t D) noexcept
{
    const std::size_t row_bytes = D * sizeof(float);
    for (std::size_t h = 0; h < H; ++h) {
        for (std::size_t t = 0; t < T; ++t) {
            std::memcpy(dst + (h * T + t) * D,
                        src + (t * H + h) * D, row_bytes);
        }
    }
}
...
CompressedKVStore<KeyBits, ValBits>::make(std::size_t head_dim,
                                          ...
                                          std::uint32_t seed) noexcept
{
    ...
    if (head_dim % value_group_size != 0)
        return make_error<CompressedKVStore>(Error::InvalidDim);
...
template <int KeyBits, int ValBits>
Error CompressedKVStore<KeyBits, ValBits>::append_chunk(
    ...
    std::size_t            n_tokens) noexcept
{
    ...
    const Error ek = prod_.quantize(
        std::span<const float>(kt.data(), HT * D), HT,
    ...
    const Error ev = Val::quantize(
        std::span<const float>(vt.data(), HT * D), HT, D, gs,
...
CompressedKVStore<KeyBits, ValBits>::get_flat() noexcept
{
    ...
    for (std::size_t h = 0; h < H; ++h) {
        ...
        for (const Chunk& ck : chunks_) {
            ...
            std::memcpy(flat_.qjl_signs.data()   + dst_off * qb,
                        ck.qjl_signs.data()      + src_off * qb, Tc * qb);
            std::memcpy(flat_.residual_norms.data() + dst_off,
                        ck.residual_norms.data()    + src_off, Tc * sizeof(float));
            std::memcpy(flat_.norms.data() + dst_off,
                        ck.norms.data()    + src_off, Tc * sizeof(float));
            std::memcpy(flat_.val_data.data()   + dst_off * vb,
                        ck.val_data.data()      + src_off * vb, Tc * vb);
            std::memcpy(flat_.val_scales.data() + dst_off * ng,
                        ck.val_scales.data()    + src_off * ng, Tc * ng * sizeof(float));
            std::memcpy(flat_.val_zeros.data()  + dst_off * ng,
                        ck.val_zeros.data()     + src_off * ng, Tc * ng * sizeof(float));
            t_off += Tc;
        }
    }
