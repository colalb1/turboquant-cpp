...
void ValuePack<8>::Policy::unpack(const std::uint8_t* in, std::size_t d, std::uint8_t* out) noexcept
{
    std::memcpy(out, in, d);
}
} // namespace detail
...
template <int Bits>
Error ValueCodec<Bits>::quantize(std::span<const float>  v,
                                 std::size_t              batch,
                                 std::size_t              dim,
                                 ...
                                 std::span<float>         zeros_out) noexcept
{
    ...
    for (std::size_t b = 0; b < batch; ++b) {
        neon::group_quant_row(v.data() + b * dim, dim, group_size, n_levels,
                              raw.data(),
                              scales_out.data() + b * ng,
                              zeros_out.data()  + b * ng);
                                   ...
                                   std::span<float>              v_out) noexcept
{
    ...
    for (std::size_t b = 0; b < batch; ++b) {
        ...
        neon::group_dequant_row(raw.data(), dim, group_size,
                                scales.data() + b * ng,
                                zeros.data()  + b * ng,
                                v_out.data()  + b * dim);
    }
