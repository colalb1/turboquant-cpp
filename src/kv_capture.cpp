...
KVCaptureEngine<KeyBits, ValBits>::make(std::size_t head_dim, std::size_t num_kv_heads,
    ...
    return Result<KVCaptureEngine>(KVCaptureEngine(std::move(*store), std::move(*ring)));
}
...
Error KVCaptureEngine<KeyBits, ValBits>::drain_ring_to_store() noexcept {
    ...
    const std::size_t drained =
        ring_.drain(std::span<float>(kbuf.data(), n * rs), std::span<float>(vbuf.data(), n * rs));
    if (drained != n) return Error::BufferTooSmall;

    return store_.append_chunk(std::span<const float>(kbuf.data(), n * rs),
                               std::span<const float>(vbuf.data(), n * rs), n);
}

template <int KeyBits, int ValBits>
Error KVCaptureEngine<KeyBits, ValBits>::ingest_prefill(std::span<const float> keys,
    ...
    const Error       ec =
        store_.append_chunk(std::span<const float>(keys.data(), n_compress * rs),
                            std::span<const float>(values.data(), n_compress * rs), n_compress);
    if (ec != Error::Ok) return ec;

    return ring_.write(std::span<const float>(keys.data() + n_compress * rs, cap * rs),
                       std::span<const float>(values.data() + n_compress * rs, cap * rs), cap);
}

template <int KeyBits, int ValBits>
Error KVCaptureEngine<KeyBits, ValBits>::ingest_decode(std::span<const float> keys,
                                                       std::span<const float> values,
                                                       std::size_t            n_tokens) noexcept {
    const std::size_t rs = ring_.row_stride();
    if (keys.size() != n_tokens * rs) return Error::ShapeMismatch;
    ...
    while (remaining > 0) {
        ...
        const Error       ew =
            ring_.write(std::span<const float>(k, n * rs), std::span<const float>(v, n * rs), n);
