...
inline double log_beta_pdf(double x, int d) noexcept {
    ...
    const double log_const =
        std::lgamma(d / 2.0) - 0.5 * std::log(M_PI) - std::lgamma((d - 1) / 2.0);
...
double mse_cost(const std::vector<double>& centroids, const std::vector<double>& boundaries, int d,
                int n_total) noexcept {
    double            cost = 0.0;
...
Result<CodebookView> compute_lloyd_max_internal(std::uint32_t dim, std::uint32_t bits,
    ...
    for (int iter = 0; iter < opts.max_iter; ++iter) {
        ...
        for (std::size_t i = 0; i < n_clusters; ++i) {
            new_centroids[i] =
                conditional_mean(boundaries[i], boundaries[i + 1], d, opts.n_quadrature);
        }
    ...
    if (!centroids_out.resize(n_clusters) || !interior_out.resize(n_clusters - 1)) {
