...
inline void gemv_rowmajor(const float* A, const float* x, float* y,
                          std::size_t d, bool accumulate) noexcept
{
...
TurboQuantProd<Bits, Arch>::make(std::size_t dim, std::uint32_t seed) noexcept
{
    if (dim == 0 || dim > kMaxDim)
        return make_error<TurboQuantProd>(Error::InvalidDim);
    ...
    if (!s.resize(dim * dim))
        return make_error<TurboQuantProd>(Error::RotationFailed);
    ...
    return Result<TurboQuantProd>(
        TurboQuantProd(dim, std::move(*mse), std::move(s), scale));
}
...
template <int Bits, ArchTag Arch>
Error TurboQuantProd<Bits, Arch>::quantize(std::span<const float>  x,
                                           std::size_t             batch,
                                           ...
                                           std::span<float>        norms_out) const noexcept
{
    ...
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                ...
                alpha,
                residual.data(), static_cast<int>(d),
                s_.data(),       static_cast<int>(d),
                beta,
                projected.data(), static_cast<int>(d));
#else
    for (std::size_t b = 0; b < batch; ++b) {
        // projected[b, i] = Σ_j residual[b, j] * S[i, j]
        gemv_rowmajor(s_.data(), residual.data() + b * d,
                      projected.data() + b * d, d, /*accumulate=*/false);
    }
#endif
                                             ...
                                             std::span<float>              x_out) const noexcept
{
    ...
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
                static_cast<int>(batch), static_cast<int>(d), static_cast<int>(d),
                alpha,
                signs.data(), static_cast<int>(d),
                s_.data(),    static_cast<int>(d),
                beta,
                qjl_contrib.data(), static_cast<int>(d));
#else
    for (std::size_t b = 0; b < batch; ++b) {
        ...
        for (std::size_t j = 0; j < d; ++j) {
            float acc = 0.0f;
            for (std::size_t i = 0; i < d; ++i) acc += sr[i] * s_[i * d + j];
            yr[j] = acc;
        }
    }
#endif
    ...
    for (std::size_t b = 0; b < batch; ++b) {
        ...
        for (std::size_t i = 0; i < d; ++i) xr[i] += scale * qr[i];
    }
...
template <int Bits, ArchTag Arch>
Error TurboQuantProd<Bits, Arch>::attention_score(
    ...
    std::span<float>              scores_out) const noexcept
{
    ...
    const Error dq = mse_.dequantize(key_mse_indices, key_norms, n_k,
                                     std::span<float>(k_mse.data(), n_k * d));
    if (dq != Error::Ok) return dq;
    ...
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                static_cast<int>(n_q), static_cast<int>(n_k), static_cast<int>(d),
                one,
                query.data(), static_cast<int>(d),
                k_mse.data(), static_cast<int>(d),
                zero,
                scores_out.data(), static_cast<int>(n_k));

    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                static_cast<int>(n_q), static_cast<int>(d), static_cast<int>(d),
                one,
                query.data(), static_cast<int>(d),
                s_.data(),    static_cast<int>(d),
                zero,
                q_sketched.data(), static_cast<int>(d));
#else
    for (std::size_t q = 0; q < n_q; ++q) {
        for (std::size_t k = 0; k < n_k; ++k) {
            float acc = 0.0f;
            for (std::size_t i = 0; i < d; ++i) acc += query[q * d + i] * k_mse[k * d + i];
            scores_out[q * n_k + k] = acc;
        }
    }
    for (std::size_t q = 0; q < n_q; ++q) {
        gemv_rowmajor(s_.data(), query.data() + q * d,
                      q_sketched.data() + q * d, d, /*accumulate=*/false);
    }
#endif
    ...
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                ...
                signs.data(),      static_cast<int>(d),
                zero,
                scores_qjl.data(), static_cast<int>(n_k));
#else
    for (std::size_t q = 0; q < n_q; ++q) {
        for (std::size_t k = 0; k < n_k; ++k) {
            float acc = 0.0f;
            for (std::size_t i = 0; i < d; ++i) acc += q_sketched[q * d + i] * signs[k * d + i];
    ...
    for (std::size_t q = 0; q < n_q; ++q) {
        for (std::size_t k = 0; k < n_k; ++k) {
            scores_out[q * n_k + k] += qjl_scale_ * key_residual_norms[k] *
                                       scores_qjl[q * n_k + k];
        }
    }
